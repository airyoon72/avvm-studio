...
const $=(s,root=document)=>root.querySelector(s);
    ...
    async function createOrder(){
      ...
      try{
        ...
        const paymentId = orderId.replace(/[^a-zA-Z0-9]/g, '').slice(0, 32);
    ...
    function showRetryButton(requestId, token, container) {
      ...
      `;
      document.getElementById('retryVideoBtn')?.addEventListener('click', async () => {
        container.innerHTML = `<div style="font-size:11px; color:rgba(255,255,255,0.6)">영상을 다시 요청하는 중... (Restarting generation...)</div>`;
        const order = JSON.parse(localStorage.getItem('avvmOrder_' + token) || '{}');
        if (order.imageData) {
          try {
            const resGen = await fetch(apiBase + '/api/generate-video', {
    ...
    function startPolling(requestId, token) {
      ...
      progressDiv.innerHTML = `
        <div style="font-size:12px; font-weight:800; color:var(--lime); margin-bottom:8px; text-transform:uppercase; letter-spacing:0.05em; display:flex; align-items:center; justify-content:center; gap:6px;">
          <span class="spinner" style="display:inline-block; width:12px; height:12px; border:2px solid var(--lime); border-top-color:transparent; border-radius:50%; animation:spin 1s linear infinite;"></span>
          AI 영상 제작 진행 중 (AI Video Generating)
        </div>
        <div style="width:100%; height:6px; background:rgba(255,255,255,0.1); border-radius:999px; overflow:hidden; margin:10px 0;">
          <div id="videoProgressBar" style="width:5%; height:100%; background:var(--lime); transition:width 0.4s ease; border-radius:999px;"></div>
        </div>
        <div id="videoProgressLabel" style="font-size:11px; color:rgba(255,255,255,0.6)">대기 열 등록 및 서버 요청 대기 중...</div>
      `;
        ...
        try {
          ...
          if (status === 'IN_QUEUE') {
            ...
            if (videoUrl) {
              ...
              localStorage.setItem('avvmOrder_' + token, JSON.stringify(order));
              showCompletedVideo(videoUrl, progressDiv);
            } else {
        ...
        service: {
          ko: `
            ...
            <ul>
              ...
              <li><b>Signature</b>: 1080p FHD 최고 화질, 정밀한 비주얼 보정 및 1:1 맞춤 연출이 결합된 프리미엄 에디션입니다.</li>
            </ul>
            <h3>작업 프로세스</h3>
          ...
          en: `
            ...
            <ul>
              ...
              <li><b>Signature</b>: Ultra-high 1080p FHD resolution, advanced visual styling, custom art direction.</li>
            </ul>
            <h3>Workflow</h3>
          ...
          ja: `
            ...
            <ul>
              ...
              <li><b>Signature</b>: 1080p FHD高解像度、精密なビジュアル補正、カスタム演出が融合したプレミアムエディションです。</li>
            </ul>
            <h3>制作の流れ</h3>
          ...
          zh: `
            ...
            <ul>
              ...
              <li><b>Signature (旗舰版)</b>: 极致 1080p FHD 分辨率，高级视觉修正，1对1专属艺术设计。</li>
            </ul>
            <h3>制作流程</h3>
...
(function(){
      ...
      document.addEventListener("click", function(e){
        ...
        if(langBtn){
          e.preventDefault();
          window.avvmLangLocked = true;
          const targetLang = langBtn.dataset.lang;
          if(window.applyAVVMLang) window.applyAVVMLang(targetLang);
          const oldBtn = document.querySelector('.lang-switcher [data-lang="'+targetLang+'"]');
          if(oldBtn) oldBtn.click();
          document.querySelectorAll(".global-lang-bar button").forEach(b=>b.classList.toggle("active", b.dataset.lang===targetLang));
        }
      }, true);
      ...
      document.addEventListener("click", function(e){
        ...
        const hidden = document.getElementById(
          type === "aspect" ? "aspectRatio" : (type === "resolution" ? "resolution" : "idSpec")
        );
      ...
      document.addEventListener("submit", function(){
        ...
        if(res) res.setAttribute("value", res.value || "540p");
      }, true);
    })();
...
(function(){
  ...
  window.getAVVMTranslation = function(lang, key) {
    const base = I18N[lang] || I18N.ko;
    return base[key] || I18N.en[key] || I18N.ko[key];
  };
})();
...
(function(){
  ...
  function startAutoplay() {
    if (window.avvmLangLocked) return;
    const currentLang = localStorage.getItem('avvmLang') || 'ko';
    cycleIdx = languages.indexOf(currentLang);
    if (cycleIdx === -1) cycleIdx = 0;

    autoplayInterval = setInterval(() => {
      if (window.avvmLangLocked) {
        clearInterval(autoplayInterval);
        return;
      }
      cycleIdx = (cycleIdx + 1) % languages.length;
      const targetLang = languages[cycleIdx];
      
      // Only translate the hero subtitle (#heroSubText) during autoplay cycle
      const heroSubEl = document.getElementById('heroSubText');
      if (heroSubEl && typeof window.getAVVMTranslation === 'function') {
        const val = window.getAVVMTranslation(targetLang, "heroSub");
        const span = heroSubEl.querySelector('span') || heroSubEl;
        if (val) span.innerHTML = val;
      }
    }, 2000);
  }
  ...
  window.addEventListener('load', () => {
    ...
    // Start autoplay cycle 2 seconds after load
